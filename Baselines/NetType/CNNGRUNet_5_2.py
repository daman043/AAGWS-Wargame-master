...
class CNNGRUNet_5_2(torch.nn.Module):
    def __init__(self, inplanes, midplanes, outplanes, in_vector, out_vector, in_rnn, out_rnn, outdense):
        ...
        self.conv5 = nn.Conv2d(midplanes, outplanes, stride=1, kernel_size=3, padding=1, bias=False)
        self.bn5 = nn.BatchNorm2d(outplanes)
    ...
    def forward(self, states_G, states_S, require_init):
        ...
        for idx, (state_S, state_G) in enumerate(zip(states_S, states_G)):
            ...
            for num in range(2, 6):
                x_s = F.relu(getattr(self, "bn{}".format(num))(getattr(self, "conv{}".format(num))(x_s)), inplace=True)

            x_s = x_s.view(-1, self.plane_length*self.outplanes)

            x_g = F.relu(self.linear_g(state_G), inplace=True)

            x = torch.cat((x_s, x_g), 1)

            x_list = [F.relu(getattr(self, "pre_fc{}".format(num))(x), inplace=True) for num in range(6)]
            ...
            x_list = [F.relu(getattr(self, "end_fc{}".format(num))(self.h2[num]), inplace=True) for num in range(6)]
